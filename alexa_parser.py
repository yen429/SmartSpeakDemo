...
def setBodyTemplate1Widget(MainWindow,jsonData):
    answerText = jsonData.get('textField','NA')
    ...
    textBrowser.append(answerText)
    print("answerLabelTotalHeight: %s"%(answerLabelTotalHeight))
    return textBrowser,answerLabelTotalHeight

def setBodyTemplate2Widget(MainWindow,jsonData):
    answerText = jsonData.get('textField','NA')
    ...
    imageUrls = [(source .get('size', 'NA'), source .get('url', 'NA')) for source in jsonData.get('image',  'NA').get('sources',  'NA')]
    url = imageUrls[0][1]
...
def setListTemplate1Widget(MainWindow,jsonData):
    itemWidget = QtWidgets.QWidget(MainWindow)
    itemWidget.setContentsMargins(0, 0, 0, 0)
    itemLayout = QtWidgets.QVBoxLayout(itemWidget)
    itemLayout.setContentsMargins(0, 0, 0, 0)
...
def parseRespondData(MainWindow,jsonData):
    ...
    if jsonData:
        ...
        if "BodyTemplate1" in type:
            itemWidget,answerItemTotalHeight = setBodyTemplate1Widget(MainWindow,jsonData)
        elif "BodyTemplate2" in type:
            itemWidget,answerItemTotalHeight = setBodyTemplate2Widget(MainWindow,jsonData)
        elif "ListTemplate1" in type:
            itemWidget,answerItemTotalHeight = setListTemplate1Widget(MainWindow,jsonData)
