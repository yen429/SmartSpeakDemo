...
class SoundDeviceStream(object):
    ...
    def __init__(self, sample_rate, sample_width, block_size, flush_size):
        ...
        self._audio_input_stream = sd.RawInputStream(
            samplerate=sample_rate, dtype=audio_format, channels=1,
            blocksize=int(block_size/2),  # blocksize is in number of frames.
        )
    ...
    def read(self, size):
        ...
        buf, overflow = self._audio_input_stream.read(size)
        if overflow:
    ...
    def write(self, buf):
        """Write bytes to the stream."""
        underflow = self._audio_output_stream.write(buf)
        if underflow:
    ...
    def flush(self):
        #print('[audio_helpers] SoundDeviceStream flush')
        if self._flush_size > 0:
            self._audio_output_stream.write(b'\x00' * self._flush_size)
            
    def startInput(self):
        #print('[audio_helpers] SoundDeviceStream startInput')
        """Start the underlying stream."""
        if not self._audio_input_stream.active:
            self._audio_input_stream.start()

    def startOutput(self):
        #print('[audio_helpers] SoundDeviceStream startOutput')
        """Start the underlying stream."""
        if not self._audio_output_stream.active:
            self._audio_output_stream.start()

    def stopInput(self):
        #print('[audio_helpers] SoundDeviceStream stopInput')
        """Stop the underlying stream."""
        if self._audio_input_stream.active:
           self._audio_input_stream.stop()

    def stopOutput(self):
        #print('[audio_helpers] SoundDeviceStream stopOutput')
        """Stop the underlying stream."""
    ...
    def closeInput(self):
        #print('[audio_helpers] SoundDeviceStream closeInput')
    ...
    def closeOutput(self):
        #print('[audio_helpers] SoundDeviceStream closeOutput')
        """Close the underlying stream and audio interface."""
...
class ConversationStream(object):
    ...
    def start_recording(self):
        #print('[audio_helpers] start_recording')
        """Start recording from the audio source."""
        self._stop_recording.clear()
        self._source.startInput()
        self._sink.startOutput()

    def stop_recording(self):
        #print('[audio_helpers] stop_recording')
        """Stop recording from the audio source."""
        self._stop_recording.set()

    def start_playback(self):
        #print('[audio_helpers] start_playback')
        """Start playback to the audio sink."""
        self._start_playback.set()
        self._source.stopInput()
        self._source.closeInput()

    def stop_playback(self):
        #print('[audio_helpers] stop_playback')
        """Stop playback from the audio sink."""
        self._start_playback.clear()
        self._sink.stopOutput()
    ...
    def closeSource(self):
        #print('[audio_helpers] ConversationStream closeSource')
        """Close source."""
        self._source.closeInput()

    def closeSink(self):
        #print('[audio_helpers] ConversationStream closeSink')
        """Close sink."""
        self._sink.closeOutput()
