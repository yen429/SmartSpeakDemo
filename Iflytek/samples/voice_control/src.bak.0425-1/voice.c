...
void voice_init(const char *xfyun_appid, const char *usb_audio_addr)
{
	...
	voice.pcm_capture_handle  = pcm_setup(SND_PCM_STREAM_CAPTURE);
	voice.pcm_playback_handle = pcm_setup(SND_PCM_STREAM_PLAYBACK);
...
static int text_to_speech(const char* src_text)
{
	...
	INFO_MSG("PCM Setup is done,size =%d, frames =%d, datablock = %d\n", size, (int)frames, datablock);
	/**********************************tts的部分************************************/
	ret = -1;
	...
	while (voice.sound_box_ongoing_flag >= 0)  //当前音箱资源没有被占用（可以播放声音）
	{
		...
		{		
			...
			while (voice.sound_box_ongoing_flag >= 0)
			{
				...
				if (voice.state_machine != RESPONSE_STATE)
				{
				...
				ret = snd_pcm_writei(voice.pcm_playback_handle, buffer, 512);
				if( ret < 0)
				{
					usleep(2000);
					if (ret == -EPIPE)
			...
			DEBUG_MSG("audio_len=%d,  pcm_size=%d\n", audio_len,pcm_size);      
		}
...
static int speech_awake(void)
{
	...
	while (voice.recongnition_switch == SWITCH_ON && (voice.state_machine == IDLE_STATE || voice.state_machine == RESPONSE_STATE) )
	{
		int ret = 0;
		//录制一次音频！大小为size(1024)
		memset(rec_pcm_buffer, 0, size);
		rc = snd_pcm_readi(voice.pcm_capture_handle, rec_pcm_buffer, frames);  //frames大小应该为512， size大小为1024
		if (rc == -EPIPE)
		{
...
static int speech_recognition(char *rec_result, int rec_result_size)
{
	...
	//const char* session_begin_params = "sub = iat, domain = music, language = zh_ch, accent = mandarin, sample_rate = 16000,sch=1, nlp_version=3.0,result_type = plain, result_encoding = utf8";
		...
		if (upload_buffer_size >= len)
		{
			...
			if (volume <= volume_threshold && front_voice_counter_ongoing == 1)
			{
				if (front_voice_zero_num >= 15)
				{
					...
					front_voice_counter_ongoing = 1;
					front_voice_zero_num = 0;
					DEBUG_MSG("front_voice_zero_num >= 15\n");
					break;			
				}
				else
				{
...
void *speech_awake_th(void *param)
{
	...
	while(1)
	{
		if(voice.state_machine == IDLE_STATE || voice.state_machine == RESPONSE_STATE)
		{
