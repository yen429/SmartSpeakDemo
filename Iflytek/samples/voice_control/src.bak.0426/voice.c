...
static int text_to_speech(const char* src_text)
{
	...
	size = frames * datablock; /*4 代表数据快长度, frames = 2048, datablock =2*/
    ...
    buffer =(char*)malloc(size);  //size为4096， frames为512
...
static int speech_recognition(char *rec_result, int rec_result_size)
{
	...
	while (voice.recongnition_switch == SWITCH_ON)
	{
		...
		//录制一次音频！大小为size(4096)
		memset(rec_pcm_buffer, 0, size);
		rc = snd_pcm_readi(voice.pcm_capture_handle, rec_pcm_buffer, frames);  //frames大小应该为2048， size大小为4096
		if (rc == -EPIPE)
		{
			ERROR_MSG("Overrun Occurred\n");
			//sleep(5);
